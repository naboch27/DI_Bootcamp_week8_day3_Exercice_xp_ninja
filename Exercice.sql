...
CREATE TABLE
    waiting_list(
        ...
        CONSTRAINT fk_inventory FOREIGN KEY(inventory_id) REFERENCES inventory(inventory_id) ON UPDATE CASCADE ON DELETE RESTRICT
    );

CREATE OR REPLACE FUNCTION FN_WAITING_LIST() RETURNS 
TRIGGER LANGUAGE PLPGSQL AS 
	'
		...
		'
	CREATE TRIGGER
	    tr_waiting_list AFTER
	UPDATE
	    ON waiting_list FOR EACH ROW
	EXECUTE
	    PROCEDURE fn_waiting_list();
	...
	INSERT INTO
	    waiting_list(complete_name, inventory_id)
	VALUES ('Enfant 1', 1), ('Enfant 2', 2), ('Enfant 3', 3), ('Enfant 4', 4), ('Enfant 5', 5);
	UPDATE waiting_list SET takes = TRUE WHERE id IN (1, 3, 5);
